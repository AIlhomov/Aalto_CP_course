// g++ -std=c++17 -O2 -pipe -Wall -Wextra forest_queries_bit2d.cpp -o forest_queries_bit2d
#include <bits/stdc++.h>
using namespace std;

struct FenwickTree2D
{
    int n, m;                // dimensions (0..n-1, 0..m-1)
    ...
    // prefix sum over rectangle [0..x][0..y]  [0-based, inclusive]
    ...
    // sum over rectangle [x1..x2][y1..y2]  [0-based, inclusive]
...
int main()
{
    ...
    // Build: add 1 at each tree cell
    for (int y = 0; y < n; ++y)
    {
        for (int x = 0; x < n; ++x)
        {
            if (g[y][x] == '*')
                ft.add(y, x, 1); // using (row=y, col=x)
        }
    }
    ...
    {
        int y1, x1, y2, x2;
        cin >> y1 >> x1 >> y2 >> x2; // input is 1-based
        // convert to 0-based
        --y1;
        --x1;
        --y2;
